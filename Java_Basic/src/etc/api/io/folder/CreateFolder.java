...
public class CreateFolder {
	...
	public static void main(String[] args) {
		...
		File file = new File("C:\\MyWork\\folder_test\\test\\mytest\\Maeil");
		
		if(!file.exists()) {//해당 파일 및 폴더가 존재하면 true, 존재하지 않다면 false
//			file.mkdir();//만들 파일 및 폴더가 1개일때만 사용한다, 만들고자 하는 경로의 상위 디렉토리가 존재하지 않는다면 동작 안함.
			file.mkdirs();//폴더생성 -> 전부 다 생성해줌.
