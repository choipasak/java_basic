...
public class HashSetExample {
	...
	public static void main(String[] args) {
        ...
        */
		...
		Set<String> set = new HashSet<>(); //다형성을 써주는게 좋음
		...
		System.out.println(set);//마지막 Java가 사라져있음
        ...
        */
		...
		for(String s : set) {
			//향상for문이 iterable?을 상속?하는데 iterater라는 객체를 이용해서 
			//계속 hasNext()를 돌리면서 값을 담아주는 거임.
		...
		//생성자의 매개값으로 set으로 지정했던 이름을 주면 set을 list로 변환 가능
		//반대로도 list의 매개값으로 set을 주면 변환 가능
		//그대신 중복가능이였던 값들 모두 지워짐.
		...
		Person kim = new Person("김춘식", 5);
		Person kim2 = new Person("김춘식", 5);
		
		personSet.add(kim);
		personSet.add(kim2);
		System.out.println(personSet);
		//결과 한개로 나옴
		//hashcode를 오버라이딩 해놨었기 때문. 내용이 같게 표현되게 작성 되있음
		//hashcode를 오버라이딩 하지 않았다면 해시알고리즘을 사용할 수 없음.
