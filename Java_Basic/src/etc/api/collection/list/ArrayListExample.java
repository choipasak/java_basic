...
public class ArrayListExample {
	...
	public static void main(String[] args) {
		...
		nick.add("짹짹이");
		nick.add("야옹이");//인덱스로 구분되기 때문에 중복 가능!
		
		//리스트 내부가 궁금하다!
		System.out.println(nick);
		//toString이 이미 오버라이딩 되어 있기때문에 바로 내용을 출력해준다.
		
		//add(인덱스, 객체): 특정 인덱스에 객체를 삽입하는 메서드
		nick.add(3, "어흥이");
		System.out.println(nick);//리스트의 길이가 동적으로 늘어나서 삽입 가능함
		
		//get(인덱스): 리스트 내부의 객체를 참조하는 메서드
//		String name = nick[3]; (x) 이건 배열의 방법
		String name = nick.get(3); // Arraylist의 방법
		...
		//그 리스트에 객체가 있으면 true, 없으면 false
		//단순 객체 존재 유무를 알고싶다면!
		System.out.println(nick.contains("멍멍이")); //true
		System.out.println(nick.contains("메뚜기")); //false
		
		//indexof(): 그 객체가 몇번째에 있는지, 특정 객체의 인덱스의 탐색!
		//만약 리스트에 없다면 -1이 온다!
		//객체가 안에 몇번째에 있는지 알고 싶다면!
		...
		//isempty(): 리스트가 비어 있는지의 여부 확인, true/false로 출력됨
		...
		//List는 객체들의 모음 저장임. 그래서 기본자료형은 안된다.
		score.add(65);//오토박싱으로 알아서 포장해서 객체 타입으로 저장된다.
		
		//한번에 객체 많이 추가하기
		//Collections의 기능을 사용하면 가능!
		Collections.addAll(score, 78, 100, 43, 56, 31);
