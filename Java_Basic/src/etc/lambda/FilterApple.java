...
public class FilterApple {
    ...
    /**
     * @solution - try1: 사과 바구니에서 녹색 사과들만 필터링 해보자!
     * @problem - 만약에 다른 색깔의 사과를 필터링 해야 한다면? (ex RED, YELLOW)
     *            새로운
     ...
     *                    - 혹시 파라미터로 함수를 전달 받을 수 있다면?
     *                    - 자바는 함수를 전달할 수 있는가?
     *                      -> 불가능하다. 자바의 함수는 일급 객체가 아닙니다.
     *                      -> 함수 개념이 아니라 객체에 소속 된 메서드 개념.
     *                    - 근데 전달되는 함수가 고정 되어 있는가?
     *                      -> 필터 조건에 따라 함수 내용이 달라야 한다.
     *                      -> 그러면 어떻게 함수 이름이 통일 되면서
     *                          다른 내용을 전달 할 수 있을까??
     *                          -> 오버라이딩 (상속, 인터페이스에서 사용 가능)
     *                          -> 인터페이스를 사용해서 오버라이딩 해보자!
     * @problem - 사과만 필터링할 수 있음,,
     *            다른 과일이나 전자 제품 같은 것들은 필터링 불가능
     */
    ...
    public static List<Apple> filterApple(List<Apple> basket, ApplePredicate predicate){
        ...
        // 반복문을 통한 필터링
        for (Apple apple : basket) {
            if (predicate.test(apple)){ // apple을 받아서 predicate가 test를 할 것임
                filteredBasket.add(apple);
            }
        }
        return filteredBasket;
    ...
    public static <T> List<T> filter(List<T> basket, GenericPredicate<T> predicate) {
        ...
        // 반복문을 통한 필터링
        for (T t : basket) {
            if (predicate.test(t)) { // apple을 받아서 predicate가 test를 할 것임
