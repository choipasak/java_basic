...
/*
# 매개 변수 (parameter[전달된 값을 받는 값], argument[전달되는 값])
...
eg) int x, int y => int start, int end
...
public class MethodExample2 {
	...
	//매개변수가 필요없는 경우
	static String selectRandomFood(){ // 매개변수를 받지 않아도 되는 메서드
	...
	static int calcNumberTotal2(int... nums) {
		// ...nums 는 앞에 ...을 붙인순간부터 여러개의 값을 받을 수 있다.
		// 굳이 배열로 선언해서 포장해서 보낼 필요가 없다!
		int total = 0;
		for(int n: nums) {
			total += n;
		}
	...
	public static void main(String[] args) {
		...
		int result = calcRangeTotal(34, 97);
		...
		sum = calcNumberTotal(new int[] {1,2,3,4,5});
		System.out.println(sum);
		//이렇게 바로 선언해서 보내줘도 가능하다!
		//왜냐면 매개변수가 배열이니까
