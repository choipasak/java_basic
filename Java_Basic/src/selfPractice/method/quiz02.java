...
public class quiz02 {
	...
	static void push(String targetName) {
		String[] temp = new String[foods.length+1];
		for(int i=0; i<foods.length; i++) {
			temp[i] = foods[i];
		}
		temp[temp.length-1] = targetName;
	...
	static void remove(String targetName) {
		...
		for(int i=idx; i<foods.length; i++) {
			if(idx == -1) {
				System.out.println("목록에 없는 음식입니다.");
			}else {
	...
	static void modify(int idx, String targetName) {
		foods[idx] = targetName;
	}
